...
function expectedImpact(annotations: Annotations | undefined): string {
  ...
  // The issue's safety split must survive into the public catalog: only the
  // non-idempotent destructive tools (DESTRUCTIVE preset — deletes, sweeps,
  // raw escape hatch) are labelled "destructive operation". Reversible updates
  // (UPDATE preset: destructive + idempotent, e.g. complete/reopen) overwrite
  // fields but converge on replay and read as plain data changes.
  if (annotations?.destructiveHint && !annotations.idempotentHint) return "destructive operation";
  return "changes data";
}
